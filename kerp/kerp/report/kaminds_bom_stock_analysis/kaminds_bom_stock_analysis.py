...
def fmt_qty(value, precision=2):
	"""Format a float quantity for display as a string, so blank rows stay blank."""
	return frappe.utils.fmt_money(value, precision=precision, currency=None)
...
def get_data_with_qty_to_make(filters):
	...
	qty_precision = frappe.get_precision("Bin", "actual_qty")
		...
		data.append(
			{
				...
				"qty_per_unit": fmt_qty(qty_per_unit, qty_precision),
				"available_qty": fmt_qty(row.actual_qty, qty_precision),
				"required_qty": fmt_qty(required_qty, qty_precision),
				"difference_qty": fmt_qty(difference_qty, qty_precision),
		...
		data.append(
			{
				...
				"description": row[2],
				"from_bom_no": row[3],
				"qty_per_unit": fmt_qty(row[4], qty_precision),
				"available_qty": fmt_qty(row[5], qty_precision),
			}
		)
...
def get_bom_data(filters):
	...
	# Pre-aggregate Bin so it contributes exactly one row per item_code,
	# regardless of how many warehouses that item has stock in.
	bin_subquery = frappe.qb.from_(bin).select(
		bin.item_code, Sum(bin.actual_qty).as_("actual_qty")
	)
	...
	if filters.get("warehouse"):
		...
		if warehouse_details:
			...
			bin_subquery = (
				bin_subquery.join(wh)
				.on(bin.warehouse == wh.name)
				.where((wh.lft >= warehouse_details.lft) & (wh.rgt <= warehouse_details.rgt))
			)
		else:
			bin_subquery = bin_subquery.where(bin.warehouse == filters.get("warehouse"))

	bin_subquery = bin_subquery.groupby(bin.item_code)
	...
	query = (
		...
		.select(
			...
			IfNull(bin_subquery.actual_qty, 0).as_("actual_qty"),
		)

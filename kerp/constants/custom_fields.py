CUSTOM_FIELDS = {
    "Item": [
        {
            "fieldname": "item_name_abbreviation_kerp",
            "label": "Item Name Abbreviation",
            "fieldtype": "Data",
            "unique": "1",
            "depends_on": "eval:doc.has_variants && doc.is_stock_item",
            "mandatory_depends_on": "eval:doc.has_variants",
            "insert_after": "item_name",
        },
        {
            "fieldname": "item_assay_value_kerp",
            "label": "Assay Value",
            "fieldtype": "Link",
            "options": "Item Assay Value",
            "depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "mandatory_depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "insert_after": "section_break_11",
        },
        {
            "fieldname": "item_variant_name_kerp",
            "label": "Variant Name",
            "fieldtype": "Link",
            "options": "Item Variant Name",
            "depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "mandatory_depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "insert_after": "section_break_11",
        },
        {
            "fieldname": "column_break_1_kerp",
            "fieldtype": "Column Break",
            "insert_after": "description",
        },
        {
            "fieldname": "brand_item_reference_kerp",
            "label": "Brand Item Reference",
            "fieldtype": "Data",
            "description": 'Type "None" if not available',
            "depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "mandatory_depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "insert_after": "brand",
        },
        {
            "fieldname": "item_pack_size_kerp",
            "label": "Pack Size",
            "fieldtype": "Link",
            "options": "Item Pack Size",
            "depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "mandatory_depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "insert_after": "brand_item_reference_kerp",
        },
        {
            "fieldname": "item_physical_form_kerp",
            "label": "Physical Form",
            "fieldtype": "Link",
            "options": "Item Physical Form",
            "depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "mandatory_depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "insert_after": "item_pack_size_kerp",
        },
        {
            "fieldname": "item_physical_sub_form_kerp",
            "label": "Physical Sub-Form",
            "fieldtype": "Link",
            "options": "Item Physical Sub-Form",
            "depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "mandatory_depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "insert_after": "item_physical_form_kerp",
        },
        {
            "fieldname": "item_grade_kerp",
            "label": "Grade",
            "fieldtype": "Link",
            "options": "Item Grade",
            "depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "mandatory_depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "insert_after": "item_physical_sub_form_kerp",
        },
        {
            "fieldname": "item_grade_standard_kerp",
            "label": "Grade Standard",
            "fieldtype": "Link",
            "options": "Item Grade Standard",
            "depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "mandatory_depends_on": "eval:!doc.has_variants && doc.is_stock_item && !doc.is_fixed_asset",
            "insert_after": "item_grade_kerp",
        },
    ],
    "Item Group": [
        {
            "fieldname": "abbreviation_kerp",
            "label": "Abbreviation",
            "fieldtype": "Data",
            "reqd": "1",
            "unique": "1",
            "insert_after": "item_group_name",
        },
    ],
    "Brand": [
        {
            "fieldname": "abbreviation_kerp",
            "label": "Abbreviation",
            "fieldtype": "Data",
            "reqd": "1",
            "unique": "1",
            "insert_after": "brand",
        },
        {
            "fieldname": "column_break_1_kerp",
            "fieldtype": "Column Break",
            "insert_after": "description",
        },
        {
            "fieldname": "manufacturers_kerp",
            "label": "Manufacturer Reference",
            "fieldtype": "Table",
            "options": "Manufacturer Reference",
            "insert_after": "column_break_1_kerp",
        },
    ],
    "Manufacturer": [
        {
            "fieldname": "abbreviation_kerp",
            "label": "Abbreviation",
            "fieldtype": "Data",
            "reqd": "1",
            "unique": "1",
            "insert_after": "full_name",
        },
    ],
    "Batch": [
        {
            "fieldname": "batch_no_kerp",
            "label": "Batch Number",
            "fieldtype": "Data",
            "read_only": 1,
            "no_copy": 1,
            "depends_on": "eval:!doc.__islocal",
            "insert_after": "sb_batch",
        },
        {
            "fieldname": "item_pack_size_kerp",
            "label": "Pack Size",
            "fieldtype": "Link",
            "options": "Item Pack Size",
            "no_copy": 1,
            "reqd": 1,
            "insert_after": "stock_uom",
        },
    ],
    "Customer": [
        {
            "fieldname": "customer_code_kerp",
            "label": "Customer Code",
            "fieldtype": "Data",
            "unique": "1",
            "read_only": "1",
            "no_copy": "1",
            "insert_after": "customer_name",
        },
        {
            "fieldname": "country_kerp",
            "label": "Country",
            "fieldtype": "Link",
            "options": "Country",
            "read_only_depends_on": "eval:!doc.__islocal",
            "mandatory_depends_on": "eval:doc.__islocal",
            "insert_after": "customer_group",
        },
    ],
    "Supplier": [
        {
            "fieldname": "supplier_code_kerp",
            "label": "Supplier Code",
            "fieldtype": "Data",
            "unique": "1",
            "read_only": "1",
            "no_copy": "1",
            "insert_after": "supplier_name",
        }
    ],
    "Address": [
        {
            "fieldname": "licenses_section_kerp",
            "label": "Address Licenses",
            "fieldtype": "Section Break",
            "insert_after": "gst_state_number",
        },
        {
            "fieldname": "licenses_kerp",
            "label": "Licenses",
            "fieldtype": "Table",
            "options": "Address License",
            "no_copy": "1",
            "insert_after": "licenses_section_kerp",
        },
    ],
    "Sales Invoice": [
        {
            "fieldname": "marine_insurance_policy_no_kerp",
            "label": "Marine Insurance Policy No.",
            "fieldtype": "Data",
            "no_copy": "1",
            "insert_after": "distance",
            "allow_on_submit": "1",
        },
        {
            "fieldname": "not_for_medical_use_kerp",
            "label": "Not For Medical Use",
            "fieldtype": "Check",
            "default": "1",
            "insert_after": "is_export_with_gst",
            "allow_on_submit": "1",
        },
        {
            "fieldname": "lr_copy_kerp",
            "label": "Transport Receipt Copy",
            "fieldtype": "Attach",
            "no_copy": "1",
            "insert_after": "marine_insurance_policy_no_kerp",
            "allow_on_submit": "1",
        },
    ],
    "Purchase Order": [
        {
            "fieldname": "transporter_name_kerp",
            "label": "Transporter Name",
            "fieldtype": "Autocomplete",
            "options": "",
            "no_copy": "1",
            "insert_after": "shipping_rule",
            "allow_on_submit": "1",
        },
        {
            "fieldname": "shipping_contact_person_kerp",
            "label": "Shipping Contact Person",
            "fieldtype": "Link",
            "options": "Contact",
            "insert_after": "shipping_address_display",
            "print_hide": 1,
        },
        {
            "fieldname": "shipping_contact_display_kerp",
            "label": "Shipping Contact",
            "fieldtype": "Small Text",
            "fetch_from": "shipping_contact_person_kerp.full_name",
            "insert_after": "shipping_contact_person_kerp",
            "read_only": 1,
        },
        {
            "fieldname": "shipping_contact_mobile_kerp",
            "label": "Shipping Contact Mobile No",
            "fieldtype": "Small Text",
            "fetch_from": "shipping_contact_person_kerp.mobile_no",
            "options": "Phone",
            "insert_after": "shipping_contact_display_kerp",
            "hidden": 1,
            "read_only": 1,
        },
        {
            "fieldname": "shipping_contact_email_kerp",
            "label": "Shipping Contact Email",
            "fieldtype": "Data",
            "options": "Email",
            "fetch_from": "shipping_contact_person_kerp.email_id",
            "insert_after": "shipping_contact_mobile_kerp",
            "hidden": 1,
            "read_only": 1,
        },
    ],
    "Sales Order": [
        {
            "fieldname": "transporter_kerp",
            "label": "Transporter",
            "fieldtype": "Link",
            "options": "Supplier",
            "no_copy": "1",
            "insert_after": "shipping_rule",
            "allow_on_submit": "1",
        },
        {
            "fieldname": "transporter_name_kerp",
            "label": "Transporter Name",
            "fieldtype": "Data",
            "no_copy": "1",
            "insert_after": "transporter_kerp",
            "allow_on_submit": "1",
        },
    ],
    "Quotation": [
        {
            "fieldname": "transporter_kerp",
            "label": "Transporter",
            "fieldtype": "Link",
            "options": "Supplier",
            "no_copy": "1",
            "insert_after": "shipping_rule",
            "allow_on_submit": "1",
        },
        {
            "fieldname": "transporter_name_kerp",
            "label": "Transporter Name",
            "fieldtype": "Data",
            "no_copy": "1",
            "insert_after": "transporter_kerp",
            "allow_on_submit": "1",
        },
    ],
}

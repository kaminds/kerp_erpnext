...
def autoname(doc, method=None):
    if len(doc.links) > 0:
        ...
        if link.link_doctype == "Customer":
            ...
            doc.name = f"{'-'.join(doc._get_full_name().split())}-{customer.customer_code_kerp}-{random_int}"
        elif link.link_doctype == "Supplier":
            supplier = frappe.get_doc("Supplier", link.link_name)
            doc.address_title = supplier.supplier_name
            doc.name = f"{'-'.join(doc._get_full_name().split())}-{supplier.supplier_code_kerp}-{random_int}"
        else:
            doc.name = f"{'-'.join(doc._get_full_name().split())}-{link.link_name}-{random_int}"
